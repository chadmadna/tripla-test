namespace :db do
  ...
  task seed_test_data: [:environment] do
    ...
    users.each do |user|
      if user.id.even?
        # Follow 2 even ids below and above
        follow_ids = ((user.id - 4)..(user.id + 4)).select { |id| id.even? && id != user.id }
      else
        # Follow 2 odd ids below and above
        follow_ids = ((user.id - 4)..(user.id + 4)).select { |id| id.odd? && id != user.id }
      end
